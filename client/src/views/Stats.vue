...
export default {
  data() {
    return {
      ...
      allGames: [],
      loading: true,
      filters: { leagueId: '', gameId: '', from: '', to: '' }
    }
  },
  setup() {
...
<style scoped>
.stats { max-width: 720px; margin: 0 auto; text-align: left; }
.hint { color: var(--muted, #666); margin-bottom: 1.5rem; }
...
.field select, .field input { padding: 0.5rem; min-width: 140px; }
.loading, .empty { margin: 2rem 0; color: var(--muted, #666); }
.score-list table { width: 100%; border-collapse: collapse; }
.score-list th, .score-list td { text-align: left; padding: 0.5rem; border-bottom: 1px solid #333; }
</style>
